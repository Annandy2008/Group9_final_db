...
def register_routes(app: Flask) -> None:
    ...
    def dashboard():
        ...
        # Revenue Trend (last 12 months, filtered)
        trend_f: list[str] = ["c.dateIncurred >= date('now', '-12 months')", _room_scope_sql]
        trend_p: list[Any] = list(_rs_params)
        if date_from:
            trend_f.append("c.dateIncurred >= ?"); trend_p.append(date_from)
        if date_to:
            trend_f.append("c.dateIncurred <= ?"); trend_p.append(date_to)
        if party_type:
            trend_f.append("p.partyType = ?"); trend_p.append(party_type)

        revenue_trend = db.execute(
            f"""
            SELECT strftime('%Y-%m', c.dateIncurred) AS month,
                   COALESCE(SUM(c.chargeAmount), 0) AS monthly_revenue
            FROM charge c
            JOIN stay s        ON s.stayId        = c.stayId
            JOIN reservation r ON r.reservationId = s.reservationId
            JOIN party p       ON p.partyId       = r.partyId
            WHERE {' AND '.join(trend_f)}
            GROUP BY month
            ORDER BY month ASC
            """,
            trend_p,
        ...
        trend_labels = [row["month"] for row in revenue_trend]
        trend_data = [row["monthly_revenue"] for row in revenue_trend]
